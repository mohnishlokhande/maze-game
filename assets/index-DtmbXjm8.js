 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const EI=Math.log(2);class SI{constructor(e){const n=u=>parseInt(Math.log(u)/EI,10),s=u=>parseInt(Array(u+1).join("1"),2);this.count=n(e+1),this.current_=this.count-1;const l=s(this.count);this.bits_=e+1&l}nextBitIsOne(){const e=!(this.bits_&1<<this.current_);return this.current_--,e}}const yl=function(r,e,n,s){r.sort(e);const l=function(p,g){const w=g-p;let y,v;if(w===0)return null;if(w===1)return y=r[p],v=n?n(y):y,new Ge(v,y.node,Ge.BLACK,null,null);{const R=parseInt(w/2,10)+p,D=l(p,R),F=l(R+1,g);return y=r[R],v=n?n(y):y,new Ge(v,y.node,Ge.BLACK,D,F)}},u=function(p){let g=null,w=null,y=r.length;const v=function(D,F){const $=y-D,ve=y;y-=D;const we=l($+1,ve),pe=r[$],le=n?n(pe):pe;R(new Ge(le,pe.node,F,null,we))},R=function(D){g?(g.left=D,g=D):(w=D,g=D)};for(let D=0;D<p.count;++D){const F=p.nextBitIsOne(),$=Math.pow(2,p.count-(D+1));F?v($,Ge.BLACK):(v($,Ge.BLACK),v($,Ge.RED))}return w},d=new SI(r.length),f=u(d);return new gt(s||e,f)};/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
